...
const ReviewsSection = () => {
  ...
  const reviews = [
    ...
    },
    {
      ...
      company: 'spice club',
      role: 'Founder',
      image: 'https://res.cloudinary.com/dxiwvcfs5/image/upload/v1773308730/CLIENT_NAMES-11_result_qixzjm.webp',
      rating: 5,
      text: 'Working with Pixdot Company has been one of the best decisions for our restaurant branding. They didn’t just create posters and menus they helped bring our vision to life.Every detail, from layout to color combination, reflected our brand personality beautifully. The finishing and print quality gave our restaurant a premium and professional touch. We truly appreciate their dedication, creativity, and smooth coordination throughout the process. If you want to take your brand to the next level, Pixdot is the team to trust!.'
    },
    {
      name: 'Hari Krishna',
      company: '2 innings sports club',
      role: 'Founder',
      image: 'https://res.cloudinary.com/dxiwvcfs5/image/upload/v1773308729/CLIENT_NAMES-01_result_wwaibo.webp',
      rating: 5,
      text: 'Good service from Pixdot, They are maintaining my page Froliic sports club in Instagram and Facebook . Gave them with 352 followers within 3 months they made to 408 followers organically with constant activity.'
    }
  ]
  ...
  const REVIEWS_PER_GRID = 3
  const gridGroupsCount = Math.ceil(reviews.length / REVIEWS_PER_GRID)
  ...
  // Auto-scroll functionality (main carousel - every 4 seconds)
  useEffect(() => {
  ...
  // "More Client Reviews" grid: rotate which 3 reviews are shown every 30 minutes (balance reviews cycle)
  useEffect(() => {
    const halfHourMs = 30 * 60 * 1000
    const interval = setInterval(() => {
      setGridStartIndex((prev) => (prev + 1) % gridGroupsCount)
    }, halfHourMs)
    return () => clearInterval(interval)
  }, [gridGroupsCount])
    ...
    <section id="reviews-section" className={`section-padding ${isDarkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-gray-50 to-white'}`}>
      <div className="container-custom">
        <div className={`text-center mb-16 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="inline-flex items-center bg-gradient-to-r from-primary to-secondary rounded-full px-6 py-3 mb-6 animate-fadeInUp">
        ...
        {/* Additional Reviews Grid (3 columns) - rotates every 30 min to show balance reviews */}
        <div className={`mt-20 transition-all duration-1000 delay-1200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <h3 className={`text-2xl font-normal ${isDarkMode ? 'text-white' : 'text-primary'} text-center mb-12 animate-fadeInUp`}>More Client Reviews</h3>
          <p className={`text-center text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} mb-8`}>
            Showing set {gridStartIndex + 1} of {gridGroupsCount} • Changes every 30 minutes
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {reviews
              .slice(gridStartIndex * REVIEWS_PER_GRID, gridStartIndex * REVIEWS_PER_GRID + REVIEWS_PER_GRID)
              .map((review, index) => (
              <div key={`${review.name}-${gridStartIndex}-${index}`} className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 animate-slideInUp`} style={{ animationDelay: `${index * 0.2}s` }}>
                <div className="flex items-center mb-4">
                  ...
                  />
                  <div>
                    <h4 className={`font-normal ${isDarkMode ? 'text-white' : 'text-primary'}`}>{review.name}</h4>
