          ...
          <nav className="hidden lg:flex space-x-6 xl:space-x-8">
            {navigation.map((item, index) => (
              <Link
                key={item.name}
                to={item.href}
                className={`font-normal transition-all duration-300 ease-out transform hover:scale-105 hover:-translate-y-0.5 ${
            ...
            <div className={`px-4 pt-4 pb-4 space-y-2 ${isDarkMode ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'} border-t shadow-lg`}>
              {navigation.map((item, index) => (
                <Link
                  key={item.name}
                  to={item.href}
                  className={`block px-4 py-3 text-base font-normal rounded-lg transition-all duration-300 ease-out transform hover:scale-105 hover:translate-x-2 ${
                    location.pathname === item.href
